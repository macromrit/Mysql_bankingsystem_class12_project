...
#crude_fun
from crude_fun import ref_all, void_sep
import datetime
...
#-------------------------------------------------------------------------------------------------------->    
    def authenticate_user(name: str, 
        ...
        try:
            check_cursor.execute(F"SELECT name, unique_id, password FROM account_holders where name like '{name}'")
        ...
        return (authenticated, name, user_id)
...
#-------------------------------------------------------------------------------------------------------->    
    def withdraw_funds(user_name: str,
        ...
        if withdrawal:
            return withdrawal,  ref_all('w', user_name, user_id, amount, upd_balance)
        else:
...
#-------------------------------------------------------------------------------------------------------->    
    def deposit_funds(user_name: str,
        ...
        if deposited: return deposited, ref_all('d', user_name, user_id, amount, upd_balance)
...
#-------------------------------------------------------------------------------------------------------->
    def transfer_funds(user_name: str,
        ...
        if 0<amount<sender_balance[0][0]:
            ...
            if transferred: return transferred, ref_all('t', user_name, user_id, amount, recievers_name, recievers_id, amount)
            ...
            if super_users[username] == passcode:
                ...
                while True:
                    ...
                    if splitter == 'g':
                        ...
                        # bargraph = pt.bar(check_list_x, check_list_y, align='center', alpha=0.2, color='red', label="customer's balance")
                        # pt.xlabel("Name")
                        # pt.ylabel("Balance")
                        # pt.legend()
                        # pt.title('users vs users via balance')
                        # pt.show()
...
#-------------------------------------------------------------------------------------------------------->
    if __name__=='__main__':
        # print(transfer_funds('C.Amrit Subramanian', '#6@GdYDo5j', 12000, 'balamurugan', '4KvkjOA6@&')[1])
        # x = transfer_funds('C.Amrit Subramanian', '#6@GdYDo5j', 1, 'balamurugan', '4KvkjOA6@&')
        # print(x[1])
        pass
else:
