...
while True:
    ...
    if desiration in list('1234'):
        ...
        if desiration=='1':#login
            ...
            authenticated, real_name, real_id = authenticate_user(name, user_id, password)
            if authenticated:
...
#transfer--------------------------------------------------------------------------------------------------->                                  
                            ...
                            recievers_id = cust_inp('Enter receever\'s bank id: ', 's') 
                            checky_cursor = connect.cursor()
                            checky_cursor.execute(F"SELECT name, unique_id from account_holders where unique_id = '{recievers_id}'")
                            main_val = [x for x in checky_cursor.fetchall()]
                            
                            if main_val:
                                if (recievers_name, recievers_id) == main_val[0] and main_val: 
                                ...
                                else: print('Ooopss.. it no such user was detectable.. Try again with valid info!!')
...
#delete--------------------------------------------------------------------------------------------------->                              
                            ...
                            assurity = cust_inp('Are ya sure ya wanna delete this account(y/n->any cahracter): ', 's').casefold()
                            if assurity=='y':
                                delete_user(real_id)
...
#display-detail-------------------------------------------------------------------------------------------->                                    
                ...
                void_sep()
                print("Username and password did'nt match!! or no such user found!!")
                func_sep()
            ...
            while True:
                ...
                print('balance initialized to 0.. deposit cash if wanted after loggin in')
                void_sep()
